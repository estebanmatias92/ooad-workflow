...
0. Bootstrap once: `/setup-ooad` (RUP/Waterfall/Agile + tracker + architecture)
1. **Need vision/PRD?** → `/ooad-discover` (RUP Inception + Vision)
2. **PRD ready, need RE?** → `/ooad-requirements` (IEEE 29148 + Gherkin + Fagan)
3. **RE baselined, need architecture?** → `/ooad-architect` (C4 + GoF + Clean 4 Layers)
4. **Architecture approved, need code?** → `/ooad-build` (Clean 4 Layers + TDD)
5. **Code slice green, need QA?** → `/ooad-verify` (BDD + Test Pyramid) — `4 ↔ 5` iterates per UC/US until green
6. **QA green, need release?** → `/ooad-ship` (Continuous Delivery + SRE)
